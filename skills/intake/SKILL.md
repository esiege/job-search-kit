...
6. **Once the master resume text is approved, immediately generate its PDF too** — don't leave the workspace with an approved `.txt` and no PDF. Copy `scripts/templates/generate_pdf_TEMPLATE.py` to `generate_pdf_general.py`, fill it in from the approved master resume, run it (output: `Resume - <Name> - General.pdf`), and log it in `PDF_GENERATION_LOG.md`. Offer this proactively rather than waiting to be asked — see `pdf-layout-standards`'s sync rule. Present the PDF for correction the same as any other; set formatting expectations up front per `CLAUDE.md`.
