...
Run intake for this person's workspace, following the `intake` skill. (`scripts/...` paths below are bundled inside this plugin — resolve against the plugin root from your session's `SessionStart` context, not relative to the workspace.)
...
2. Dispatch the `intake-research-agent` subagent to extract and organize raw text from everything in `Intake/`. Include the resolved plugin root path (from your own session's `SessionStart` context) explicitly in the dispatch prompt — the subagent doesn't automatically inherit it, and it needs it to find `scripts/intake/extract_*.py`.
