...
1. Confirm the workspace folder (create `Job Descriptions/`, `Resumes/`, `Interview Prep/`, `Intake/` if they don't exist yet, and copy `scripts/templates/PDF_GENERATION_LOG_TEMPLATE.md` to `PDF_GENERATION_LOG.md` if it doesn't exist yet) and unzip the provided material into `Intake/` if it isn't already unpacked.
