...
4. Stop and wait. Do not save `Job Descriptions/<Company>.txt` or `Resumes/Resume - <Name> - <Company>.txt`, and do not generate a PDF, until the user explicitly approves this draft. Both files are written together only at that point, once approved. If `JOB_SEARCH_LOG.md` has a row for this company (i.e. it came from `/find-jobs`), update its status to `Resume Created` at this same point — no separate step, no reminder needed.
