...
3. Tier 1a: for known companies, query `scripts/job_search/query_job_boards.py`'s `"companies"` config directly. Tier 1b: same script's `"aggregators"` config (Remotive, RemoteOK, The Muse, and Adzuna if the person has configured Adzuna credentials) for keyword-first discovery when there's no company list yet — enable whichever aggregators make sense rather than always enabling all of them. Don't enable the `jsearch` aggregator (LinkedIn/Indeed, via third-party scraping) unless the person has explicitly opted into that specific tradeoff — see the `job-search` skill's guardrails. Tier 2: `WebSearch`/`WebFetch` fallback for anything not resolvable via a structured API. See the `job-search` skill for exact invocation and config shape. Keep searching — multiple companies, multiple queries — until a reasonable shortlist exists, not just one call's worth of results.
